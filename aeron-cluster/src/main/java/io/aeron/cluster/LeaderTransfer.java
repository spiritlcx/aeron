...
/**
 * Transfer leadership form current leader to another node.
 */
public class LeaderTransfer {
    ...
    private final long stepDownDeadlineNs;
    ...
    private int waiting() {
        ...
        if (logPosition == newLeader.logPosition()) {
            if (consensusPublisher.timeoutNow(newLeader.publication(), leadershipTermId, newLeader.id(), false)) {
                for (ClusterMember member : activeMembers) {
                    if (member.id() != newLeader.id() && !member.isLeader()) {
                        consensusPublisher.timeoutNow(member.publication(), leadershipTermId, member.id(), true);
                    }
                }
                state = NOTIFIED;
            }
